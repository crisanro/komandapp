...
import { notificarPedidoListo, notificarCuenta } from "@/lib/notificaciones";
...
type Sesion = { id: string; estado: string; pedidos: Pedido[] };
type Mesa   = { id: string; nombre: string; descripcion: string | null; capacidad: number | null; estado: string; sesiones: Sesion[] };
...
export default function MesasOperativoClient({
  mesasIniciales, restaurantId, userId, nombreMesero,
  ...
  const handleMesaUpdate = useCallback((data: unknown) => {
    const { mesaId, estado } = data as { mesaId: string; estado: string };
  ...
  const handleItemUpdate = useCallback((data: unknown) => {
    const { mesaId, sesionId, pedidoId, itemId, estado } = data as {
      mesaId: string; sesionId: string; pedidoId: string; itemId: string; estado: string;
    };
    if (estado === "LISTO") notificarPedidoListo("");
        ...
        sesiones: m.sesiones.map(s =>
          s.id === sesionId ? {
            ...
            pedidos: s.pedidos.map(p =>
              p.id === pedidoId ? {
                ...p,
                items: p.items.map(i => i.id === itemId ? { ...i, estado } : i),
              } : p
            ),
          } : s
        ),
    ...
    for (const mesa of mesas) {
      for (const sesion of mesa.sesiones) {
        if (cuentasSolicitadas.has(sesion.id)) {
          lista.push({ tipo: "cuenta", mesaId: mesa.id, mesaNombre: mesa.nombre, sesionId: sesion.id, detalle: "Pide la cuenta" });
          continue;
        }
        ...
        if (listos.length > 0) {
          lista.push({
            tipo: "listo", mesaId: mesa.id, mesaNombre: mesa.nombre, sesionId: sesion.id,
            detalle: listos.length === 1 ? `${listos[0].menuItem?.nombre ?? "1 ítem"} listo` : `${listos.length} ítems listos para llevar`,
          });
        } else if (enPrep.length > 0) {
          lista.push({
            tipo: "preparacion", mesaId: mesa.id, mesaNombre: mesa.nombre, sesionId: sesion.id,
  ...
  async function handleAbrirMesa(mesaId: string) {
    ...
    if (result?.ok && result.sesionId) router.push(`/mesa/${result.sesionId}`);
  }
          ...
          <section className="p-3 pb-0">
            ...
            <div className="space-y-2">
              {pendientes.map((p, i) => {
                ...
                return (
                  <button key={i} onClick={() => router.push(`/mesa/${p.sesionId}`)}
            ...
            <div className="grid-mesas">
              {mesas.map(mesa => {
                ...
                return (
                  <button key={mesa.id}
                    onClick={() => ocupada
                      ? router.push(`/mesa/${mesa.sesiones[0].id}`)
