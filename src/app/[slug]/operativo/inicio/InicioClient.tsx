...
type ItemSinEstacion = {
  ...
  pedido: {
    ...
    sesion: { id: string; token?: string } | null;
  } | null;
};
...
export default function InicioClient({
  nombre, permisos, estaciones, restaurantId, restaurantSlug,
      ...
      <div className="p-4 space-y-5 pb-28">
        ...
        {alertas.length > 0 && (
          <div className="space-y-2">
            ...
            {alertas.map(a => {
              ...
              // Buscar el token correspondiente a la sesión
              const mesaConSesion = mesas.find(m => m.sesiones.some(s => s.id === a.sesionId));
              const token = mesaConSesion?.sesiones.find(s => s.id === a.sesionId)?.token;
              ...
              return (
                <div key={a.id}
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {token && (
                      <button onClick={() => router.push(`/${restaurantSlug}/operativo/mesa/${token}`)}
          ...
          <div>
            ...
            <div className="space-y-2">
              {itemsDespacho.map(item => {
                const sesionId = item.pedido?.sesion?.id;
                const mesaConSesion = mesas.find(m => m.sesiones.some(s => s.id === sesionId));
                const token = mesaConSesion?.sesiones.find(s => s.id === sesionId)?.token ?? item.pedido?.sesion?.token;
                ...
                return (
                  <div key={item.id}
                    ...
                    {token && (
                      <button
                        onClick={() => router.push(`/${restaurantSlug}/operativo/mesa/${token}`)}
        ...
        {puedeMesas && (
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => router.push(`/${restaurantSlug}/operativo/mesas`)}
            ...
            {mesasOcupadas.length === 0 ? (
              <div className="rounded-xl p-6 text-center"
                ...
                {permisos.puedeAbrirMesas && (
                  <button onClick={() => router.push(`/${restaurantSlug}/operativo/mesas`)} className="btn btn-primary btn-sm">
                    Abrir mesa
                  </button>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {mesasOcupadas.map(m => {
                  ...
                  return (
                    <button key={m.id}
                      onClick={() => router.push(`/${restaurantSlug}/operativo/mesa/${sesion.token}`)}
        ...
        {puedeKds && (
          <div>
            ...
            <button onClick={() => router.push(`/${restaurantSlug}/operativo/kds`)}
        ...
        {(puedeMesas || puedeCaja) && (
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {puedeCaja && (
                <button onClick={() => router.push(`/${restaurantSlug}/operativo/caja`)}
                  className="rounded-xl p-4 text-left"
                  style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
