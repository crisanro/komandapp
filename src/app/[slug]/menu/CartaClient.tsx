...
type Restaurant = {
  ...
  color: string;
  logoUrl: string | null;
  notasMenu: string | null;
  notaMenuEn?: string | null;
};
...
// Helper resumen para la carta:
function resumenPromoCliente(p: Promo): string {
  switch (p.tipo) {
    case "PORCENTAJE":
      return `${p.porcentaje}% de descuento en tu cuenta`;
    case "PORCENTAJE_CATEGORIA":
      return `${p.porcentaje}% en ${p.categoria?.nombre ?? "esta categoría"}`;
    case "MONTO_FIJO":
      return `$${p.montoFijo} de descuento`;
    case "2X1":
      return `2x1 en ${p.menuItem?.nombre ?? "ítem seleccionado"}`;
    case "3X2":
      return `3x2 en ${p.menuItem?.nombre ?? "ítem seleccionado"}`;
    case "COMBO":
      return `Combo por $${p.precioCombo} — ${p.itemsCombo
        .map((i) => `${i.cantidad}x ${i.menuItem?.nombre}`)
        .join(", ")}`;
    case "HAPPY_HOUR":
      return `${p.porcentaje}% de descuento de ${p.horaInicio} a ${p.horaFin}`;
    case "PRIMERA_VISITA":
      return `${p.porcentaje}% en tu primera visita`;
    case "CUMPLEANOS":
      return `${p.porcentaje}% de descuento en tu cumpleaños`;
    default:
        ...
        <div className="relative px-5 pt-12 pb-8 flex items-start justify-between">
          <div>
            {restaurant.logoUrl ? (
              ...
              <div
                className="w-16 h-16 rounded-2xl mb-4 flex items-center justify-center text-2xl font-bold border-2 border-white/20"
          ...
          {tieneIngles && (
            <button
              ...
              className="mt-2 px-3 py-1.5 rounded-lg text-xs font-semibold border-2 border-white/30 text-white hover:bg-white/10 transition-colors shrink-0"
            >
              {idioma === "es" ? "🇺🇸 EN" : "🇪🇨 ES"}
            </button>
          )}
            ...
            >
              ...
              <div className="flex-1">
                ...
                {promo.horaInicio && promo.horaFin && (
                  <p className="text-xs mt-1" style={{ color: color + "cc" }}>
                    🕐 {promo.horaInicio} – {promo.horaFin}
                  </p>
                )}
              </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="px-4 pb-20">
            ...
            {restaurant.notasMenu && (
              <p className="text-xs text-gray-600 text-center pt-4 pb-8">
                {restaurant.notasMenu}
              </p>
            )}
          </div>
